...
mutable struct GeneralizedMinimalResidual{M, MP1, MMP1, T, AT} <:
    ...
    function GeneralizedMinimalResidual(
        ...
        krylov_basis = ntuple(i -> similar(x), M + 1)
        H = @MArray zeros(M + 1, M)
        g0 = @MArray zeros(M + 1)

        new{M, M + 1, M * (M + 1), eltype(x), AT}(
            krylov_basis,
            H,
            g0,
            rtol,
...
function initialize!(
    ...
    @assert size(x) == size(krylov_basis[1])

    # store the initial residual in krylov_basis[1]
    linearoperator!(krylov_basis[1], x, args...)
...
function doiteration!(
    ...
    Ω = LinearAlgebra.Rotation{eltype(x)}([])
    j = 1
    for outer j in 1:M
    ...
    converged || initialize!(linearoperator!, x, b, solver, args...)
